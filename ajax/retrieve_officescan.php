...
if (($handle = fopen($file, "r")) !== FALSE) {
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
			...
			if($num >= 70){
				...
				$sql = "insert into antivirus_client_list(ClientName,IP,DomainLevel,ConnectionState,GUID,ScanMethod,DLPState,VirusNum,SpywareNum,OS,BitVersion,MAC,ClientVersion,VirusPatternVersion) values('".$status['ClientName']."','".$status['IP']."','".$status['DomainLevel']."','".$status['ConnectionState']."','".$status['GUID']."','".$status['ScanMethod']."','".$status['DLPState']."','".$status['VirusNum']."','".$status['SpywareNum']."','".$status['OS']."','".$status['BitVersion']."','".$status['MAC']."','".$status['ClientVersion']."','".$status['VirusPatternVersion']."')
				ON DUPLICATE KEY UPDATE ClientName = '".$status['ClientName']."',IP = '".$status['IP']."',DomainLevel = '".$status['DomainLevel']."',ConnectionState = '".$status['ConnectionState']."',ScanMethod = '".$status['ScanMethod']."',DLPState = '".$status['DLPState']."',VirusNum = '".$status['VirusNum']."',SpywareNum = '".$status['SpywareNum']."',OS = '".$status['OS']."',BitVersion = '".$status['BitVersion']."',MAC = '".$status['MAC']."',ClientVersion = '".$status['ClientVersion']."',VirusPatternVersion = '".$status['VirusPatternVersion']."'";
