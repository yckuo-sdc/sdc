			...
			<div class="post vul_overview">
				...
				<div class="post_cell">
                    <div class="ui secondary pointing menu">
                        <?php foreach($local_gov_vuls as $index => $local_gov_vul): ?>
                            <?php if ($index == 0): ?>
                                <?php $a_class = "active item" ?>
                            <?php else: ?> 
                                <?php $a_class = "item" ?>
                            <?php endif ?>
                            <a class="<?=$a_class?>"><?=$local_gov_vul['displayname']?></a>
                        <?php endforeach ?>
                    </div>
                    <div class="ui noborder bottom attached segment">
                        <?php foreach($local_gov_vuls as $index => $local_gov_vul): ?>
                            <?php if ($index == 0): ?>
                                <?php $tab_class = $local_gov_vul['id'] . " show" ?>
                            <?php else: ?> 
                                <?php $tab_class = $local_gov_vul['id'] ?>
                            <?php endif ?>
                            <div class="tab-content <?=$tab_class?>">
                                共有 <?=$local_gov_vul['ou_number']?> 個組織單位(含歷史資料)!<br><br>
                                <div class='ui cards'>
                                    <?php foreach($local_gov_vul['ou_vuls'] as $ou_vul): ?>
