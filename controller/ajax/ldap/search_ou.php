...
$config_array = array(
    ...
    array(
        ...
        'move_ou' => 'ou=TainanComputer, dc=tainan, dc=gov, dc=tw',
        'type' => 'editcomputerou',
    )
);
...
foreach($config_array as $config):
    ...
    $data_array = array();
    $data_array['base'] = $config['search_ou'];
    $data_array['filter'] = $filter;
    $data = $ld->getData($data_array);

    $data_array = array();
    $data_array['base'] = $config['move_ou'];
    ...
    <p class="header"><?=count($data)?> entries returned from <?=$config['search_ou']?></p>
            ...
            <div class='ui ordered list'>
                <?php foreach($entry as $key => $val): ?>
                    <?php if ($key == "distinguishedname") : ?>
                        <?php $ou_description = $ld->getAllOUDescription($config['search_ou'], $val); ?>
