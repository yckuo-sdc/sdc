<?php 
$sql = "SELECT ou, SUM(total_VUL) AS total_VUL, SUM(fixed_VUL) AS fixed_VUL, IFNULL(SUM(fixed_VUL)*100.0 / SUM(total_VUL), 0) AS total_completion, SUM(total_high_VUL) AS total_high_VUL, SUM(fixed_high_VUL) AS fixed_high_VUL, IFNULL(SUM(fixed_high_VUL)*100.0 / SUM(total_high_VUL), 0) AS high_completion, SUM(total_VUL - overdue_high_VUL - overdue_medium_VUL) AS non_overdue_VUL, IFNULL(SUM(total_VUL - overdue_high_VUL - overdue_medium_VUL)*100.0 / SUM(total_VUL), 0) AS non_overdue_completion FROM view_system_vuls GROUP BY ou ORDER BY total_completion ASC";
